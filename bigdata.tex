...
At present, a handful of large service providers (i.e.\ Amazon, Microsoft, Google and IBM) hold a significant share of the market, each with their own set of branded products and proprietary ecosystem. We argue that there is greater potential benefit in opening up this marketplace.
...
One of the main challenges with trading cloud easily as a commodity on a trading platform is the lack of standardisation in industry (i.e. apples to apples). Cloud providers each have different ways in processing workloads and apply different pricing structures. The objective would be to be able to trade IT in similar fashion as Energy or Oil, but this is a challenge at the moment due to low levels of standardisation. Cloud Exchange [1] is a step in the right direction, however further is required to truly reach this goal.

Use of dynamic scaling and allocations [4] of cloud infrastructure, together with some sort of smart system that decides where to purchase the extra capacity necessary and then in turn deploy and spin up the additional instances in the purchased pay-as-you-go cloud space. 

Could progress towards a trading platform which allows anyone to setup and start selling cloud; also a rating system or index which would calculate a value/rating for reliability and performance against each cloud provider.
